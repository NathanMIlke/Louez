...
export function getBlockingReservationStatuses(pendingBlocksAvailability: boolean) {
  return pendingBlocksAvailability
    ? [...BLOCKING_RESERVATION_STATUSES]
    : (["confirmed", "ongoing"] as BlockingReservationStatus[]);
}
...
export function buildReservationOverlapPredicate(params: {
  ...
  if (!predicate) throw new Error("Failed to build reservation overlap predicate");
  return predicate;
}
